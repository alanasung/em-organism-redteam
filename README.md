<p align="center">
  ...
  <p align="center">SPAR Fall 2026 · <a href="https://github.com/alanasung/em-organism-redteam">Stress-Testing Emergent-Misalignment Model Organisms</a></p>
</p>
...
**Stress-Testing Emergent-Misalignment Model Organisms**. The shared engineering spine (config, caching, hooks, metrics,
...
| Stage | Module | Output |
|---|---|---|
| Compose config | `configs/` + `emred.configs` | resolved `config.yaml` |
| Build data | `emred.data` | splits + manifest |
| Load model | `emred.models` | `LoadedModel` + resolved commit |
| Run stages | `scripts/run_experiment.py` | per-stage JSON |
| Aggregate | `emred.reporting` | `results.json` + tables + figures |
...
├── src/emred/       # installable library (print-free)
...
- SPAR Fall 2026 project posting: https://github.com/alanasung/em-organism-redteam
...
```bibtex
@misc{emred_spar_f26,
  title        = {Stress-Testing Emergent-Misalignment Model Organisms},
  author       = {Alana Sung},
  year         = {2026},
  howpublished = {SPAR Fall 2026},
  note         = {Red-team RL-trained emergent-misalignment organisms and measure which failure modes survive simple probes.},
  url          = {https://github.com/alanasung/em-organism-redteam}
}
```
...
Ablations live under `src/emred/ablation/` and return structured dicts. Presets
