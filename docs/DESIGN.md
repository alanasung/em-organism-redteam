# Design Document: Stress-Testing Emergent-Misalignment Model Organisms

## 1. Purpose

This document is the method contract for **Stress-Testing Emergent-Misalignment Model Organisms**. It specifies what is
...
One-liner: Red-team RL-trained emergent-misalignment organisms and measure which failure modes survive simple probes.

Hypothesis: Red-team RL-trained emergent-misalignment organisms and measure which failure modes survive simple probes.
...
|---|---|---|
| Config schema | `emred.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `emred.configs.loader` | Hydra compose API |
| Device | `emred.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `emred.models.registry` | Pinned revisions + hardware fit |
| Hooks | `emred.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `emred.models.generation` | Chat template path recording |
| Data | `emred.data` | Synthetic items, splits, manifests |
| Evaluation | `emred.evaluation` | Metrics, failure harness, protocols |
| Ablation | `emred.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `emred.reporting` | Aggregate → MD/TeX/figures |
| Cache | `emred.cache` | Versioned artifact store |
...
Mentor(s): Alana Sung
Affiliation(s): 
SPAR posting: https://github.com/alanasung/em-organism-redteam
...
`src/emred/emred/` are preserved across regeneration.
